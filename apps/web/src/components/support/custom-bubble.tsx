...
export const CustomBubbleLanding = ({ className }: BubbleSlotProps) => (
	<Primitive.Bubble
		className={cn(
			className,
...
export const CustomBubbleDashboard = ({ className }: BubbleSlotProps) => (
	<Primitive.Bubble
		className={cn(
			"group/btn relative flex h-8 cursor-pointer items-center gap-2 rounded-sm bg-background-200 px-2 text-primary hover:bg-background-300"
		)}
	>
		{({ isOpen, isTyping, unreadCount }) => (
			<>
				<AnimatePresence mode="wait">
					{isOpen ? (
						...
						<motion.div
							...
							key="chat"
						>
							<Icon className="size-4" filledOnHover name="chat" />
						</motion.div>
					)}
				</AnimatePresence>
