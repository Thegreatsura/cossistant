...
import { ConversationTimelineType, MessageVisibility } from "@cossistant/types";
import { createTimelineItemSchema } from "@cossistant/types/api/timeline-item";
...
export type CreateTimelineItemOptions = {
  ...
  item: {
    ...
    visibility?:
      | typeof MessageVisibility.PUBLIC
      | typeof MessageVisibility.PRIVATE;
    createdAt?: Date;
  };
};
...
export async function createTimelineItem(
  ...
  const [createdItem] = await db
    ...
    .values({
      ...
      conversationId,
      organizationId,
      visibility: item.visibility ?? MessageVisibility.PUBLIC,
      type: item.type,
      text: item.text ?? null,
      parts: item.parts as unknown,
  ...
  const parsedItem = createTimelineItemSchema.parse({
    ...createdItem,
