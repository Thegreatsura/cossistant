...
import { routeEvent, type EventContext } from "@api/ws/router";
import {
...
function extractVisitorId(data: unknown): string | null {
        ...
        if ("message" in data) {
                const nested = (data as { message?: { visitorId?: unknown } | null }).message?.visitorId;
        ...
        if ("conversation" in data) {
                ...
                const visitorId = (conversation as { visitorId?: unknown } | null)?.visitorId;
...
export class RealtimeEmitter {
        async emit<TType extends RealtimeEventType>(
                ...
                const event: RealtimeEvent<TType> = {
                        ...
                        organizationId,
                        visitorId:
                                options.visitorId ?? extractVisitorId(data) ?? null,
                };
