...
type UpdateMemberNotificationSettingsParams = GetMemberNotificationSettingsParams & {
        settings: Array<{
...
export async function getMemberNotificationSettings(
        ...
        const rows = await db
                ...
                .where(
                        and(
                                ...
                                eq(
                                        memberNotificationSetting.organizationId,
        ...
        const settings: MemberNotificationPreference[] =
                MEMBER_NOTIFICATION_CHANNEL_DEFINITIONS.map((definition) => {
                        ...
                        if (!stored) {
                                return getDefaultMemberNotificationPreference(
                                        definition.channel
                                );
                        }
                        ...
                        return {
                                ...
                                supportsDelaySeconds:
                                        definition.supportsDelaySeconds,
...
export async function updateMemberNotificationSettings(
        ...
        return db.transaction(async (tx) => {
                for (const setting of params.settings) {
                        ...
                        const priority =
                                setting.priority ?? definition.defaultPriority;
