...
export const MessageGroup = React.forwardRef<HTMLDivElement, MessageGroupProps>(
	({ children, className, asChild = false, messages = [], viewerId, viewerType, seenByIds = [], lastReadMessageIds, ...props }, ref) => {
		...
		const hasBeenSeenByViewer = viewerId ? seenByIds.includes(viewerId) : undefined;
...
export interface MessageGroupHeaderProps
	extends Omit<React.HTMLAttributes<HTMLDivElement>, "children"> {
	children?: React.ReactNode | ((props: { name?: string; senderId?: string; senderType?: SenderType }) => React.ReactNode);
	asChild?: boolean;
	className?: string;
	name?: string;
	senderId?: string;
...
>(({ children, className, asChild = false, name, senderId, senderType, ...props }, ref) => {
...
export interface MessageGroupSeenIndicatorProps
	extends Omit<React.HTMLAttributes<HTMLDivElement>, "children"> {
	children?: React.ReactNode | ((props: { seenByIds: string[]; hasBeenSeen: boolean }) => React.ReactNode);
	asChild?: boolean;
	className?: string;
	seenByIds?: string[];
}
...
>(({ children, className, asChild = false, messageId, lastReadMessageIds, ...props }, ref) => {
	...
	if (lastReaderIds.length === 0 && !children) {
		return null;
	}
