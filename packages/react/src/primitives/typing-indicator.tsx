...
const names = participants.map((participant) => participant.name || "Someone");
...
<AvatarFallback
className="text-xs font-medium text-co-foreground"
...
<p className="text-xs font-medium text-co-foreground/80">{text}</p>
...
{preview ? (
<p className="mt-2 line-clamp-2 text-sm text-co-foreground/90">{preview}</p>
) : null}
</div>
</div>
</div>
