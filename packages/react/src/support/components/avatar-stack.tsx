			...
			props: {
				...
				children: items.map((item, index) => (
					...
					>
						{item.type === "human" && (
							<Avatar
								className={cn("size-full shadow-sm")}
								image={item.agent.image}
								name={item.agent.name}
							/>
						)}
						{item.type === "count" && (
							<div className="flex size-full items-center justify-center rounded-full bg-co-background-200 font-medium text-co-text-900 text-sm shadow-sm dark:bg-co-background-500">
								+{item.count}
							</div>
						)}
						{item.type === "ai" && (
							<div className="flex size-full items-center justify-center rounded-full bg-co-background-200 shadow-sm dark:bg-co-background-600">
								<CossistantLogo className="size-4" />
							</div>
						)}
					</AvatarStackItem>
				)),
