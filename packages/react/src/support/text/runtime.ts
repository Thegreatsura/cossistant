...
export type NormalizedOverrides = Map<SupportTextKey, NormalizedOverride<SupportTextKey>>;
...
const TIME_OF_DAY_THRESHOLDS: Array<{ token: SupportTimeOfDayToken; hour: number }> = [
    { token: "morning", hour: 9 },
    { token: "afternoon", hour: 15 },
    { token: "evening", hour: 20 },
];

function buildDayPeriodLabels(locale: string): Record<SupportTimeOfDayToken, string> {
...
export function createTextUtils(locale: string): SupportTextUtils {
...
export function buildLocaleChain(preferences: Array<string | null | undefined>): string[] {
...
export function normalizeOverrides(
    ...
    for (const key of Object.keys(overrides) as SupportTextKey[]) {
        ...
        for (const [locale, localizedValue] of Object.entries(value)) {
            ...
            byLocale.set(locale.toLowerCase(), localizedValue as SupportLocaleMessages[typeof key]);
        }
