...
export type RealtimeEventHandlerEntry<TType extends RealtimeEventType, TContext> =
...
export function useRealtime<
        TContext = void,
        THandlers extends RealtimeEventHandlersMap<TContext> = RealtimeEventHandlersMap<TContext>,
>({
        events,
        websiteId,
        visitorId,
        ...
        const errorHandlerRef = useRef<
                ((error: unknown, event: RealtimeEvent<RealtimeEventType>) => void) | undefined
        >(onEventError);
                        ...
                        for (const handler of payload) {
                                try {
                                        ...
                                        if (errorHandler) {
                                                ...
                                                console.error(
                                                        "[Realtime] Event handler threw an error",
                                                        error
                                                );
                                        }
                                }
