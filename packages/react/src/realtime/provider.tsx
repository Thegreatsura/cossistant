...
/**
 * Extracts a number field from an unknown object.
 */
...
function constructRealtimeEvent(parsed: unknown): AnyRealtimeEvent | null {
	...
	const timestamp = extractNumberField(parsed, "timestamp") ?? Date.now();
	const organizationId = extractStringField(parsed, "organizationId", true);
	const websiteId = extractStringField(parsed, "websiteId", true);
