...
function applyPage(
        ...
        page: Pick<ConversationMessagesState, "messages" | "hasNextPage" | "nextCursor">
): MessagesState {
...
function removeMessage(
	...
	const nextMessages = existing.messages.slice(0, index).concat(existing.messages.slice(index + 1));
...
export type MessagesStore = Store<MessagesState> & {
	...
	finalizeMessage(conversationId: string, optimisticId: string, message: Message): void;
};

export function createMessagesStore(initialState: MessagesState = INITIAL_STATE): MessagesStore {
	...
	return {
		...
		removeMessage(conversationId, messageId) {
			store.setState((state) => removeMessage(state, conversationId, messageId));
		},
		finalizeMessage(conversationId, optimisticId, message) {
			store.setState((state) => finalizeMessage(state, conversationId, optimisticId, message));
		},
	};
}
