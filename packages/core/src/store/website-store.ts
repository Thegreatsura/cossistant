...
export function createWebsiteStore(initialState: WebsiteState = INITIAL_STATE): WebsiteStore {
        ...
        return {
                ...
                setWebsite(website) {
                        store.setState((state) => {
                                if (state.website === website && state.status === "success" && state.error === null) {
                                        return state;
                                }
                ...
                setError(error) {
                        ...
                        store.setState((state) => {
                                if (state.status === "error" && state.error?.message === normalized.message) {
                                        return state;
                                }
