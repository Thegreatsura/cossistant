...
describe("messages store", () => {
        it("ingests pages and merges messages", () => {
                ...
                expect(store.getState().conversations[firstPage.conversationId]?.messages).toHaveLength(1);
                ...
                const conversation = store.getState().conversations[firstPage.conversationId];
		...
		it("finalizes optimistic messages by replacing them", () => {
			...
			const conversation = store.getState().conversations[optimistic.conversationId];
			expect(conversation?.messages).toHaveLength(1);
...
describe("CossistantClient message integration", () => {
        ...
        it("updates the store after fetching messages", async () => {
                ...
                await client.getConversationMessages({ conversationId: message.conversationId });

                const stored = client.messagesStore.getState().conversations[message.conversationId];
                expect(stored?.messages[0]).toEqual(message);
        });

		it("optimistically adds a message and replaces it with the server response", async () => {
			const serverMessage = createMockMessage({ id: "msg-server", bodyMd: "Server" });
			...
			client.restClient = {
				sendMessage: async (payload: SendMessageRequest) => {
					...
					stateDuringCall = client.messagesStore.getState().conversations[serverMessage.conversationId]?.messages;
			...
			const stored = client.messagesStore.getState().conversations[serverMessage.conversationId];
		...
		it("rolls back the optimistic message when the request fails", async () => {
			...
			client.restClient = {
				sendMessage: async () => {
					stateDuringCall = client.messagesStore.getState().conversations[conversationId]?.messages;
					throw error;
				},
			};
			...
			const stored = client.messagesStore.getState().conversations[conversationId];
        ...
        it("initiates a local conversation with default messages", () => {
                const defaultMessage = createMockMessage({ id: "msg-default", conversationId: "conv-local" });
                ...
                const conversation = client.conversationsStore.getState().byId["conv-local"];
                expect(conversation?.visitorId).toBe("visitor-init");
                const messages = client.messagesStore.getState().conversations["conv-local"];
        ...
        it("creates the conversation on the server when sending the first message", async () => {
                ...
                const storedConversation = client.conversationsStore.getState().byId[conversationId];
                expect(storedConversation).toEqual(serverConversation);
                const storedMessages = client.messagesStore.getState().conversations[conversationId];
        ...
        it("restores pending state when server creation fails", async () => {
                ...
                const defaultMessage = createMockMessage({ id: "msg-default", conversationId, bodyMd: "Hello" });
