...
export class CossistantRestClient {
	...
	private normalizeVisitorResponse(
		...
		return {
			...
			blockedAt: payload.blockedAt ? new Date(payload.blockedAt) : null,
		};
	}
	...
	async identify(params: {
		...
		return {
			contact: {
				...
				createdAt: new Date(response.contact.createdAt),
				updatedAt: new Date(response.contact.updatedAt),
			},
			...
			conversation: {
				...
				lastMessage: response.conversation.lastMessage
					? {
							...
							createdAt: new Date(response.conversation.lastMessage.createdAt),
							updatedAt: new Date(response.conversation.lastMessage.updatedAt),
							deletedAt: response.conversation.lastMessage.deletedAt
								? new Date(response.conversation.lastMessage.deletedAt)
								: null,
						}
					: undefined,
			},
				...
				lastMessage: conv.lastMessage
					? {
							...
							deletedAt: conv.lastMessage.deletedAt
								? new Date(conv.lastMessage.deletedAt)
								: null,
						}
					: undefined,
			})),
